App = {
    ...
    bindEvents: function() {
      $(document).on('click', '.click', App.handleTransfer);
    },
    ...
    handleTransfer: function(event) {
      ...
      var ReadLetterInstance;
        App.contracts.Timecap.deployed().then(function(instance) {
          ...
          //get title and answers of capsule
         var empnum = $("#empnum").text();
         var id = parseInt($(event.target).closest('span').text().split('.')[0],10)-1;
         //var readd = $(event.target).closest('span').next().next().closest('span');
         console.log(empnum);
         console.log(id);
         //console.log(readd);
         ...
         return ReadLetterInstance.openCap(empnum, id).then(()=>{
           
          instance.OpenEvent().watch((err1, res1) => {
              var temp = [res1.args.title, res1.args.ans];
              $.ajax({
                url: "http://localhost:8080/capsule/read/view",
                method: "POST",
                async: true,
                data: {boxes: temp},
                success: function(){
